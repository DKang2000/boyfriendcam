...
Plan a new native iOS rewrite at the repository root only. The shipping stack must be Swift + SwiftUI, AVFoundation, Vision human body pose, Core Motion, and PhotoKit only for explicit user save/export. Do not keep React Native, Expo, or JavaScript in the shipping path. Do not do a one-shot rewrite, do not create a hybrid path, and do not broaden scope beyond parity with the current RN app.
